...
local function createVehZones(shopName, entity)
    if not Config.UsingTarget then
        for i = 1, #Config.Shops[shopName]['ShowroomVehicles'] do
            -- Ensure coordinates exist for the current vehicle
            local vehicleCoords = Config.Shops[shopName]['ShowroomVehicles'][i]['coords']
            if vehicleCoords then
                ...
                print("Warning: Vehicle coordinates missing for vehicle", i, "in shop", shopName)
            end
        end
        ...
        if #zones > 0 then
            ...
            combo:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    -- Ensure PlayerData, PlayerData.job, and insideShop are valid
                    if PlayerData and PlayerData.job and insideShop and Config.Shops[insideShop] then
                        ...
                        print("Warning: PlayerData, PlayerData.job, or insideShop is invalid")
                    end
                else
                    exports['qb-menu']:closeMenu()
                end
            ...
            print("Warning: No zones were created for shop", shopName)
        end
    else
        -- Ensure entity and insideShop are valid
        if entity and insideShop and Config.Shops[insideShop] then
            ...
            print("Warning: Invalid entity or insideShop")
        end
    end
end
