...
function OpenMenu()
    QBCore.Functions.TriggerCallback("qb-scenes:server:isVip", function(cb)
        if cb then
            showMenu = true
            SetNuiFocus(true, true)
            SendNUIMessage({ action = "open"}) 
        else
            QBCore.Functions.Notify("You do not have access to StylesRP scenes, visit the tebex store to gain access to scenes", "error", 10000)
        end
    end)
end
...
function ToggleCreationLaser(data)
    ...
    QBCore.Functions.TriggerCallback("qb-scenes:server:isVip", function(cb)
        if cb then
            ...
            QBCore.Functions.Notify("You are not VIP", "error", 5000)
        end
    end)
end
