...
local function GetDepotVehicles(citizenid, state, garage, cb)
    local result = MySQL.Sync.fetchAll("SELECT * FROM player_vehicles WHERE citizenid = @citizenid AND state = @state AND (garage = @garage OR garage IS NULL OR garage = '')", {
        ['@citizenid'] = citizenid,
        ['@state'] = state,
        ['@garage'] = garage
    })
    if result and #result > 0 then
        ...
        print("Error: No vehicles found for citizen ID: " .. citizenid .. ", state: " .. state .. ", garage: " .. garage)
        cb({})
    end
end
...
RegisterNetEvent('qb-garage:server:updateVehicleState', function(state, plate, garage)
    -- Check if state, plate, and garage are not nil
    if not state or not plate then
        print("Error: Missing state or plate for updating vehicle state.")
        return
    end

    -- Set garage to 'nil' or empty string if not provided
    garage = garage or ''
    
    -- Execute SQL query to update vehicle state
    MySQL.update('UPDATE player_vehicles SET state = ?, garage = ?, depotprice = ? WHERE plate = ?', 
        { state, garage, 0, plate }, function(affectedRows)
        
        -- Check if the update was successful
        if affectedRows > 0 then
            print("Vehicle with plate: " .. plate .. " successfully updated to state: " .. state .. " in garage: " .. garage)
        else
            print("Error: Vehicle with plate " .. plate .. " not found or update failed.")
        end
    end)
end)
     ...
     MySQL.query('SELECT * FROM player_vehicles WHERE plate = ?', {vehicle.plate}, function(result)
        if result[1] then
            ...
            MySQL.Async.fetchAll('SELECT * FROM player_vehicles WHERE plate = ?', {vehicle.plate}, function(result)
                if result[1] then
                    local depotPrice = result[1].depotprice ~= 0 and result[1].depotprice
                    if depotPrice ~= (QBCore.Shared.Vehicles[result[1].vehicle].price * 0.05) or 0 then
                        depotPrice = QBCore.Shared.Vehicles[result[1].vehicle].price * 0.05
                    end
                    if cashBalance >= depotPrice then
                        Player.Functions.RemoveMoney("cash", depotPrice, "paid-depot")
                        exports['qb-management']:AddMoney("doj", depotPrice)
