...
QBCore.Functions.CreateCallback('qb-bossmenu:server:GetEmployees', function(source, cb, jobname)
	...
	local employees = {}
	local players = exports['srp_multijob']:getAllEmployees(jobname)

	for citizenID, rank in pairs(players) do
		...
		if isOnline then
			employees[#employees+1] = {
				...
				grade = {
					level = rank,
					name  = QBCore.Shared.Jobs[jobname].grades[tostring(rank)].name, 
				},
				isboss = QBCore.Shared.Jobs[jobname].grades[tostring(rank)].isboss,
				name = '🟢 ' .. isOnline.PlayerData.charinfo.firstname .. ' ' .. isOnline.PlayerData.charinfo.lastname
			}
		else
			local offlinePlayer = MySQL.query.await("SELECT * FROM `players` WHERE `citizenid`=? LIMIT 1", {citizenID})

			if offlinePlayer and offlinePlayer[1] then 
				employees[#employees+1] = {
					empSource = citizenID,
					grade = {
						level = rank,
					...
					isboss = QBCore.Shared.Jobs[jobname].grades[tostring(rank)].isboss,
					name = '❌ ' ..  json.decode(offlinePlayer[1].charinfo).firstname .. ' ' .. json.decode(offlinePlayer[1].charinfo).lastname
				}
			end
		end
	end
	...
	table.sort(employees, function(a, b)
		return tonumber(a.grade.level) > tonumber(b.grade.level)
	end)
...
RegisterNetEvent('qb-bossmenu:server:GradeUpdate', function(data)
	...
	if Employee then
		local prevRank = Employee.PlayerData.job.grade.level

		if Employee.Functions.SetJob(Player.PlayerData.job.name, data.grade) then
			if data.grade > prevRank then
				...
				TriggerClientEvent('QBCore:Notify', src, "Successfully demoted!", "success")
				TriggerClientEvent('QBCore:Notify', Employee.PlayerData.source, "You have been demoted to " ..data.gradename..".", "success")
			end

			exports["srp_multijob"]:AddJob(Employee.PlayerData.citizenid, Player.PlayerData.job.name, data.grade)
		else
...
RegisterNetEvent('qb-bossmenu:server:HireEmployee', function(recruit)
	...
	if Target and Target.Functions.SetJob(Player.PlayerData.job.name, 0) then
		TriggerClientEvent('QBCore:Notify', src, "You hired " .. (Target.PlayerData.charinfo.firstname .. ' ' .. Target.PlayerData.charinfo.lastname) .. " to " .. Player.PlayerData.job.label .. "", "success")
		...
		exports["srp_multijob"]:AddJob(Target.PlayerData.citizenid, Player.PlayerData.job.name, 0)
	end
