...
function setupTarget()
    CreateThread(function()
        ...
        local coords = vec3(901.34, -170.06, 74.08)
        local heading = 228.81
        ...
        -- Optional animation or scenario
        TaskStartScenarioInPlace(ped, 'WORLD_HUMAN_AA_COFFEE', 0, true)

        -- Add the target using ox_target
        exports['ox_target']:addLocalEntity(ped, {
            {
                ...
                onSelect = function()
                    TriggerEvent('qb-taxijob:client:requestcab')
                end,
                canInteract = function(entity, distance, coords, name, bone)
                    -- Optional job check
                    local playerJob = ESX.PlayerData.job.name -- Adjust according to your framework
                    return playerJob == 'taxi'
                end
            }
        })
