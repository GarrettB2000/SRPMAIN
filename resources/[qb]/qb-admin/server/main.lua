...
RegisterNetEvent("919-admin:server:AdminChatSend", function(message)
    ...
    if AdminPanel.HasPermission(src, "adminchat") then
        ...
        -- Ensure AdminChat is initialized as a table
        AdminPanel.AdminChat = AdminPanel.AdminChat or {}
...
RegisterNetEvent("919-admin:server:SendReport", function(subject, info, type)
    ...
    if Player then
        ...
        -- Ensure AdminPanel.Reports is initialized as a table
        AdminPanel.Reports = AdminPanel.Reports or {}

        -- Count the number of reports from the same citizen
        ...
        -- Check if the player has reached the maximum report limit
        ...
        -- Notify the player that their report was sent
        ...
        table.insert(AdminPanel.Reports, {
            ReportID = #AdminPanel.Reports + 1,
            Claimed = nil,
            ReportTime = os.time(),
        ...
        -- Notify admins with the permission to view reports
        for k, v in pairs(GetPlayers()) do
            v = tonumber(v)
...
RegisterServerEvent("qb-log:server:CreateLog", function(name, title, color, message, tagEveryone)
    ...
    InterceptedLogs = InterceptedLogs or {}  -- Ensure InterceptedLogs is initialized
