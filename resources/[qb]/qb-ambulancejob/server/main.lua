...
RegisterNetEvent('hospital:server:TreatWounds', function(playerId)
	...
	if Patient then
		if Player.PlayerData.job.name == 'ambulance' then
			...
			TriggerClientEvent('ox_lib:notify', src, {type = 'success', description = 'Bandage used'})
...
RegisterNetEvent('hospital:server:RevivePlayer', function(playerId, isOldMan)
	...
	if Patient then
		if Player.PlayerData.job.name == 'ambulance' or exports.ox_inventory:Search(src, 'count', 'firstaid') >= 1 then
			if oldMan then
				if Player.Functions.RemoveMoney('cash', 5000, 'revived-player') then
					exports.ox_inventory:RemoveItem(src, 'firstaid', 1)
					TriggerClientEvent('ox_lib:notify', src, {type = 'success', description = 'First aid kit used'})
			...
			MySQL.insert('INSERT INTO bans (name, license, discord, ip, reason, expire, bannedby) VALUES (?, ?, ?, ?, ?, ?, ?)', {
				...
				'Trying to revive themselves or other players',
				2147483647,
				'qb-ambulancejob'
			})
			TriggerEvent('qb-log:server:CreateLog', 'ambulancejob', 'Player Banned', 'red', string.format('%s was banned by %s for %s', GetPlayerName(src), 'qb-ambulancejob', 'Trying to revive themselves or other players'), true)
...
RegisterNetEvent('hospital:server:removeBandaid', function()
	local Player = QBCore.Functions.GetPlayer(source)
	if not Player then return end
	exports.ox_inventory:RemoveItem(source, 'bandaid', 1)
end)

RegisterNetEvent('hospital:server:removeEtgcstim', function()
	local Player = QBCore.Functions.GetPlayer(source)
	if not Player then return end
	exports.ox_inventory:RemoveItem(source, 'etgcstim', 1)
end)

RegisterNetEvent('hospital:server:removeIfaks', function()
	local Player = QBCore.Functions.GetPlayer(source)
	if not Player then return end
	exports.ox_inventory:RemoveItem(source, 'ifaks', 1)
end)

RegisterNetEvent('hospital:server:removePainkillers', function()
	local Player = QBCore.Functions.GetPlayer(source)
	if not Player then return end
	exports.ox_inventory:RemoveItem(source, 'painkillers', 1)
end)
...
QBCore.Functions.CreateUseableItem('bandaid', function(source, item)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	if Player.Functions.GetItemByName(item.name) ~= nil then
		TriggerClientEvent('hospital:client:UseBandaid', src)
	end
end)

QBCore.Functions.CreateUseableItem('etgcstim', function(source, item)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	if Player.Functions.GetItemByName(item.name) ~= nil then
		TriggerClientEvent('hospital:client:UseEtgcstim', src)
	end
end)
