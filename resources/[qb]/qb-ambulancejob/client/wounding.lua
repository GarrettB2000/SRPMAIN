...
RegisterNetEvent('hospital:client:UseBandaid', function()
    local ped = PlayerPedId()
    QBCore.Functions.Progressbar('use_bandage', Lang:t('progress.bandaid'), 4000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
        ...
        StopAnimTask(ped, 'anim@amb@business@weed@weed_inspecting_high_dry@', 'weed_inspecting_high_base_inspector', 1.0)
        TriggerServerEvent('hospital:server:removeBandaid')
        TriggerEvent('qb-inventory:client:ItemBox', QBCore.Shared.Items['bandaid'], 'remove')
...
RegisterNetEvent('hospital:client:UseEtgcstim', function()
    local ped = PlayerPedId()
    QBCore.Functions.Progressbar('use_bandage', Lang:t('progress.etgcstim'), 4000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
        ...
        StopAnimTask(ped, 'anim@amb@business@weed@weed_inspecting_high_dry@', 'weed_inspecting_high_base_inspector', 1.0)
        TriggerServerEvent('hospital:server:removeEtgcstim')
        TriggerEvent('qb-inventory:client:ItemBox', QBCore.Shared.Items['etgcstim'], 'remove')
        SetEntityHealth(ped, GetEntityHealth(ped) + 10)
        if math.random(1, 100) < 50 then
            RemoveBleed(2)
        end
