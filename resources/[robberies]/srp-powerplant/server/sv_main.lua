...
local CheckStationHits = function(playerId)
    ...
    if Config.PowerStations[1].hit and Config.PowerStations[2].hit and Config.PowerStations[3].hit and Config.PowerStations[4].hit then
        ...
        -- east powerplant hit
        TriggerEvent("srp-powerplant:server:City", playerId)
        Wait(1400)
        -- blackout
        ...
        print("^3[srp-powerplant] ^5East Power Plant Hit^7")
    ...
    --[[ elseif Config.PowerStations[5].hit and Config.PowerStations[6].hit and Config.PowerStations[7].hit then
        ...
        TriggerClientEvent('chatMessage', -1, "[LS Water & Power]", "error", "City wide power outage, we are working on it!")
        TriggerClientEvent("police:client:DisableAllCameras", -1)
        print("^3[srp-powerplant] ^5City Power Plant Hit^7")
        TriggerEvent("qb-log:server:CreateLog", "powerplants", "Blackout", "black", "**City Powerplant Hit**") ]]
    end
end

RegisterNetEvent('srp-powerplant:server:RemoveThermite', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end

    -- exports['qb-inventory']:RemoveItem(Player.PlayerData.source, 'thermite', 1, false)
    exports.ox_inventory:RemoveItem(Player.PlayerData.source, 'thermite', 1, false)
    TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['thermite'], "remove", 1)
end)

RegisterServerEvent('srp-powerplant:server:SetStationStatus', function(key, isHit)
    local src = source
    Config.PowerStations[key].hit = isHit
    TriggerEvent("qb-log:server:CreateLog", "powerplants", "Thermite", "red", "Station: "..key.." is hit successfully by "..GetPlayerName(src))
    print("^3[srp-powerplant] ^5Station: "..key.." is hit successfully by "..GetPlayerName(src).."^7")
    TriggerClientEvent("srp-powerplant:client:SetStationStatus", -1, key, isHit)
    CheckStationHits(src)
end)

RegisterServerEvent('srp-powerplant:server:City', function(playerId)
    TriggerClientEvent('srp-powerplant:client:City', -1)
...
RegisterServerEvent('srp-powerplant:server:East', function(playerId)
    TriggerClientEvent('srp-powerplant:client:East', -1)
end)

RegisterServerEvent('srp-powerplant:server:ThermitePtfx', function(coords)
    TriggerClientEvent('srp-powerplant:client:ThermitePtfx', -1, coords)
end)

QBCore.Functions.CreateCallback('srp-powerplant:server:GetConfig', function(source, cb)
    cb(Config)
end)

QBCore.Functions.CreateCallback('srp-powerplant:server:getCops', function(source, cb)
	local amount = 0
...
CreateThread(function()
    while true do
        ...
        if blackoutActive and (cityHit or eastHit) then
            Wait(1000*60*20) -- 20 MINUTES Cooldown
            ...
            if cityHit then
                ...
                TriggerClientEvent("srp-powerplant:client:SetStationStatus", -1, 5, false)
                TriggerClientEvent("srp-powerplant:client:SetStationStatus", -1, 6, false)
                TriggerClientEvent("srp-powerplant:client:SetStationStatus", -1, 7, false)
                if Config.Artgallery then
                ...
                print("^3[srp-powerplant] ^5Reset City Power Plant^7")
            elseif eastHit then
                eastHit = false
                ...
                TriggerClientEvent("srp-powerplant:client:SetStationStatus", -1, 1, false)
                TriggerClientEvent("srp-powerplant:client:SetStationStatus", -1, 2, false)
                TriggerClientEvent("srp-powerplant:client:SetStationStatus", -1, 3, false)
                TriggerClientEvent("srp-powerplant:client:SetStationStatus", -1, 4, false)
                print("^3[srp-powerplant] ^5Reset East Power Plant^7")
            end
        end
    end
end)
...
QBCore.Commands.Add("powerplant1", "Test Powerplant (God Only)", {}, false, function(source, args)
    ...
    TriggerClientEvent("srp-powerplant:client:SetStationStatus", -1, 5, true)
    Config.PowerStations[6].hit = true
    TriggerClientEvent("srp-powerplant:client:SetStationStatus", -1, 6, true)
    Config.PowerStations[7].hit = true
    TriggerClientEvent("srp-powerplant:client:SetStationStatus", -1, 7, true)
...
QBCore.Commands.Add("powerplant2", "Test Powerplant (God Only)", {}, false, function(source, args)
    ...
    TriggerClientEvent("srp-powerplant:client:SetStationStatus", -1, 1, true)
    Config.PowerStations[2].hit = true
    TriggerClientEvent("srp-powerplant:client:SetStationStatus", -1, 2, true)
    Config.PowerStations[3].hit = true
    TriggerClientEvent("srp-powerplant:client:SetStationStatus", -1, 3, true)
    Config.PowerStations[4].hit = true
    TriggerClientEvent("srp-powerplant:client:SetStationStatus", -1, 4, true)
    CheckStationHits(source)
end, "god")
