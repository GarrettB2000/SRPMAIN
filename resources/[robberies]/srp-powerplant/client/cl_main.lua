...
local ThermiteEffect = function()
    ...
    Wait(1500)
    TriggerServerEvent("srp-powerplant:server:ThermitePtfx", ptfx)
    Wait(500)
...
RegisterNetEvent('srp-powerplant:client:thermite', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
    --[[ local hasItem = QBCore.Functions.HasItem("thermite")
    if hasItem then ]]
    if exports.ox_inventory:Search('count', 'thermite', false) >= 1 then
        if currentStation ~= 0 then
            ...
            if dist < 1.20 then
                QBCore.Functions.TriggerCallback('srp-powerplant:server:getCops', function(cops)
                    if cops >= Config.MinimumThermitePolice then
                        if not Config.PowerStations[currentStation].hit then
                            TriggerServerEvent("srp-powerplant:server:RemoveThermite")
                            ...
                            --[[ exports["memorygame"]:thermiteminigame(Config.ThermiteSettings.correctBlocks, Config.ThermiteSettings.incorrectBlocks, Config.ThermiteSettings.timetoShow, Config.ThermiteSettings.timetoLose, function()
                                if currentStation ~= 0 then
                                    TriggerServerEvent("srp-powerplant:server:SetStationStatus", currentStation, true)
                                end
                                ThermiteEffect()
                            end, function()
                            ...
                            exports['ps-ui']:Scrambler(function(success)
                                if success then
                                    ...
                                    QBCore.Functions.Notify("You failed to place the thermite!", 'error', 5000)
                                end
                            end, "numeric", 20, 0) -- Type (alphabet, numeric, alphanumeric, greek, braille, runes), Time (Seconds), Mirrored (0: Normal, 1: Normal + Mirrored 2: Mirrored only )
                        else
                ...
                if cops >= Config.MinimumThermitePolice then
                    if not Config.PowerStations[currentStation].hit then
                        ...
                        exports["memorygame"]:thermiteminigame(Config.ThermiteSettings.correctBlocks, Config.ThermiteSettings.incorrectBlocks, Config.ThermiteSettings.timetoShow, Config.ThermiteSettings.timetoLose, function()
                            ...
                            ThermiteEffect()
                            if currentStation ~= 0 then
                                TriggerServerEvent("srp-powerplant:server:SetStationStatus", station, true)
                            end
                        end, function()
                            QBCore.Functions.Notify("Thermite failed..", "error", 2500)
                        end)
...
RegisterNetEvent("srp-powerplant:client:ThermitePtfx", function(coords)
    RequestNamedPtfxAsset("scr_ornate_heist")
...
RegisterNetEvent('srp-powerplant:client:SetStationStatus', function(key, isHit)
    Config.PowerStations[key].hit = isHit
end)

RegisterNetEvent('srp-powerplant:client:City', function()
...
RegisterNetEvent('srp-powerplant:client:East', function()
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    QBCore.Functions.TriggerCallback('srp-powerplant:server:GetConfig', function(config)
        Config = config
    end)
end)
        ...
        if k == 1 or k == 2 then
            exports['qb-target']:AddBoxZone("PowerPlantStation"..k, v.coords, 0.5, 0.65, {
                ...
                options = { 
                    {
                        type = "client",
                        event = "srp-powerplant:client:thermite",
