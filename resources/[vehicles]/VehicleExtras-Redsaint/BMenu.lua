...
-- Deprecated
BMenu.debug = false
function BMenu.SetDebugEnabled(enabled)
end
function BMenu.IsDebugEnabled()
	return false
end
---
...
local defaultStyle = {
	...
	titleBackgroundColor = { 255, 0, 0, 255},
	titleBackgroundSprite = nil,
	subTitleColor = { 255, 0, 0, 255 },
...
function BMenu.CreateMenu(id, title, subTitle, style)
	-- Default settings
	local menu = { }
...
function BMenu.CreateSubMenu(id, parent, subTitle, style)
	local parentMenu = menus[parent]
	if not parentMenu then
		return
	end

	BMenu.CreateMenu(id, parentMenu.title, subTitle and string.upper(subTitle) or parentMenu.subTitle)
...
function BMenu.CurrentMenu()
	return currentMenu and currentMenu.id or nil
end

function BMenu.OpenMenu(id)
...
function BMenu.IsMenuOpened(id)
	return currentMenu and currentMenu.id == id
end
BMenu.Begin = BMenu.IsMenuOpened

function BMenu.IsAnyMenuOpened()
	return currentMenu ~= nil
end

function BMenu.IsMenuAboutToBeClosed()
	return currentMenu and currentMenu.aboutToBeClosed
end

function BMenu.CloseMenu()
	if not currentMenu then
		return
	end
...
function BMenu.ToolTip(text, width, flipHorizontal)
	if not currentMenu then
		return
	end
...
function BMenu.Button(text, subText)
	if not currentMenu then
		return
	end
...
function BMenu.SpriteButton(text, dict, name, r, g, b, a)
	if not currentMenu then
		return
	end

	local pressed = BMenu.Button(text)
...
function BMenu.InputButton(text, windowTitleEntry, defaultText, maxLength, subText)
	if not currentMenu then
		return
	end

	local pressed = BMenu.Button(text, subText)
	local inputText = nil
...
function BMenu.MenuButton(text, id, subText)
	if not currentMenu then
		return
	end
...
function BMenu.CheckBox(text, checked, callback)
	if not currentMenu then
		return
	end
	...
	local pressed = BMenu.SpriteButton(text, 'commonmenu', name)
...
function BMenu.ComboBox(text, items, currentIndex, selectedIndex, callback)
	if not currentMenu then
		return
	end
	...
	local pressed = BMenu.Button(text, selectedItem)
...
BMenu.End = BMenu.Display

function BMenu.CurrentOption()
...
function BMenu.IsItemSelected()
	return currentKey == keys.select and BMenu.IsItemHovered()
end

function BMenu.SetTitle(id, title)
	setMenuProperty(id, 'title', title)
end
BMenu.SetMenuTitle = BMenu.SetTitle

function BMenu.SetSubTitle(id, text)
	setMenuProperty(id, 'subTitle', string.upper(text))
end
BMenu.SetMenuSubTitle = BMenu.SetSubTitle

function BMenu.SetMenuStyle(id, style)
	setMenuProperty(id, 'style', style)
end

function BMenu.SetMenuX(id, x)
	setStyleProperty(id, 'x', x)
end

function BMenu.SetMenuY(id, y)
	setStyleProperty(id, 'y', y)
end

function BMenu.SetMenuWidth(id, width)
	setStyleProperty(id, 'width', width)
end

function BMenu.SetMenuMaxOptionCountOnScreen(id, count)
	setStyleProperty(id, 'maxOptionCountOnScreen', count)
end

function BMenu.SetTitleColor(id, r, g, b, a)
	setStyleProperty(id, 'titleColor', { r, g, b, a })
end
BMenu.SetMenuTitleColor = BMenu.SetTitleColor

function BMenu.SetMenuSubTitleColor(id, r, g, b, a)
	setStyleProperty(id, 'subTitleColor', { r, g, b, a })
end

function BMenu.SetTitleBackgroundColor(id, r, g, b, a)
	setStyleProperty(id, 'titleBackgroundColor', { r, g, b, a })
end
BMenu.SetMenuTitleBackgroundColor = BMenu.SetTitleBackgroundColor

function BMenu.SetTitleBackgroundSprite(id, dict, name)
	RequestStreamedTextureDict(dict)
	setStyleProperty(id, 'titleBackgroundSprite', { dict = dict, name = name })
end
BMenu.SetMenuTitleBackgroundSprite = BMenu.SetTitleBackgroundSprite

function BMenu.SetMenuBackgroundColor(id, r, g, b, a)
	setStyleProperty(id, 'backgroundColor', { r, g, b, a })
end

function BMenu.SetMenuTextColor(id, r, g, b, a)
	setStyleProperty(id, 'textColor', { r, g, b, a })
end

function BMenu.SetMenuSubTextColor(id, r, g, b, a)
	setStyleProperty(id, 'subTextColor', { r, g, b, a })
end

function BMenu.SetMenuFocusColor(id, r, g, b, a)
	setStyleProperty(id, 'focusColor', { r, g, b, a })
end

function BMenu.SetMenuFocusTextColor(id, r, g, b, a)
	setStyleProperty(id, 'focusTextColor', { r, g, b, a })
end

function BMenu.SetMenuButtonPressedSound(id, name, set)
