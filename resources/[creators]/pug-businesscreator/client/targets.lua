...
RegisterNetEvent('Pug:client:OpenTocingMenu', function(Data)
    ...
    if Config.Menu == "ox_lib" then
		...
		if StockedData then
			local Image = false
			local Icon = "fa-solid fa-cart-shopping"
		...
		if StockedData then
			for k, v in pairs(StockedData) do
				...
				menu[#menu + 1] = {
					...
					image = 'https://cfx-nui-ox_inventory/web/images/'..v.item..'.png',
...
RegisterNetEvent('Pug:client:AddItemsToShop', function(Data)
    ...
    local function hasEnoughItem(selectedItem, amount)
        ...
        elseif Config.InventoryType == "qs-inventory" then
			...
			return count and count >= amount
        elseif Config.InventoryType == "codem-inventory" then
            local item = exports['codem-inventory']:getUserItem(src, selectedItem)
...
RegisterNetEvent("Pug:Client:DoBusinessMenuimageLogic", function(Data)
	...
	if Info.Image ~= nil then
		AddPropToPlayer("prop_cs_magazine", 6286, 0.15, 0.03, -0.040, 0.0, 180.0, 90.0, PlayerPedId())
