...
QBCore.Commands.Add("invoice", Loc[Config.Lan].command["charge"], {}, false, function(source) TriggerClientEvent("jim-payments:client:PolCharge", source) end)
...
RegisterServerEvent("jim-payments:server:PolPopup", function(data)
    ...
    local billed = QBCore.Functions.GetPlayer(src)
    local biller = QBCore.Functions.GetPlayer(tonumber(data.biller))
    local bankBalance = billed.PlayerData.money["bank"]
	...
	if data.accept == true then
		if bankBalance >= data.amount then
			...
			triggerNotify(nil, "You do not have enough money in your bank to pay this invoice!", "error", src)
			triggerNotify(nil, billed.PlayerData.charinfo.firstname.." can\'t afford to pay "..data.amount, "error", data.biller)
		end
	else
