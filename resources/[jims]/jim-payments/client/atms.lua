...
RegisterNetEvent('jim-payments:Client:ATM:use', function(data)
	...
	if data.id == "atm" then
		setoptions = { { value = "withdraw", label = Loc[Config.Lan].menu["withdraw"] }, }
		...
		setinputs = {
			{ type = 'select', label = setview, options = setoptions },
			{ type = 'number', isRequired = true, label = Loc[Config.Lan].menu["header_trans_amount"] }
		}
	...
	elseif data.id == "bank" then
		setoptions = { { value = "withdraw", label = Loc[Config.Lan].menu["withdraw"] }, { value = "deposit", label = Loc[Config.Lan].menu["deposit"] } }
	...
	elseif data.id == "transfer" then
		setoptions = { { value = "transfer", label = Loc[Config.Lan].menu["transfer"] } }
		...
		setinputs = {
			{ type = 'select', label = setview, options = setoptions },
			{ type = 'number', isRequired = true, label = Loc[Config.Lan].menu["header_trans_amount"] },
			{ type = 'text', isRequired = true, label = Loc[Config.Lan].menu["header_account_no"] }
		}
	...
	elseif data.id == "gangtransfer" then
		...
		setinputs = {
			{ type = 'select', label = setview, options = setoptions },
			{ type = 'number', isRequired = true, label = Loc[Config.Lan].menu["header_trans_amount"] },
			{ type = 'input', isRequired = true, label = Loc[Config.Lan].menu["header_account_no"] }
		}
	...
	QBCore.Functions.Progressbar("accessing_atm", bartext, atmbartime, false, true, { disableMovement = false, disableCarMovement = false, disableMouse = false, disableCombat = false, }, {}, {}, {}, function()
		local dialog = lib.inputDialog(setheader, setinputs)
		if dialog then
			...
			unloadAnimDict('amb@prop_human_atm@male@enter')
			Wait(1000)
			TriggerServerEvent('jim-payments:server:ATM:use', dialog[2], dialog[1], dialog[3], data.id, info.society, info.gsociety)
		end
...
RegisterNetEvent('jim-payments:client:ATM:give', function()
	...
	QBCore.Functions.TriggerCallback('jim-payments:MakePlayerList', function(cb) onlineList = cb if onlineList[1] == nil then Wait(200) end
		...
		local dialog = lib.inputDialog(Loc[Config.Lan].menu["give_cash"], {
			{ type = 'select', label = "Citizen", options = nearbyList },
			{ type = "number", label = Loc[Config.Lan].menu["amount_pay"], isRequired = true }
		})
		if dialog then
			if not dialog[1] or not dialog[2] then return end
			TriggerServerEvent('jim-payments:server:ATM:give', dialog[1], dialog[2])
		end
	end)
end)
