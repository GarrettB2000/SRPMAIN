Core = {
    ...
    StashAddItem = function(storageId, item, amount)
        ...
        elseif Config.Inventory == 'core-inventory' then
            success = exports["core_inventory"]:addItem('stash-'..storageId:gsub('/',''):gsub(':',''):gsub('#',''), item, tonumber(amount), {}, 'stash')
        elseif Config.Inventory == 'custom' then 
        ...
        if ESX ~= nil then
            ...
            for _, xPlayer in pairs(xPlayers) do
                table.insert(players, {
                    ...
                    char = Core.Player.GetFullName(xPlayer.source)
                })
            end
        elseif QBCore ~= nil then
            ...
            for _, xPlayer in pairs(xPlayers) do
                table.insert(players, {
                    ...
                    char = Core.Player.GetFullName(xPlayer.PlayerData.source)
                })
            end
        end
        return players
    ...
    CreateJob = function(tb)
        if ESX ~= nil then
            if not ESX.DoesJobExist(tb.name, 0) then
                ESX.CreateJob(tb.name, tb.label, tb.grades)
                Core.GetJobs()
                while not ESX.DoesJobExist(tb.name, 0) do 
                    Wait(10)
                end
                Core.GetJobs()
        ...
        elseif QBCore ~= nil then
            if QBCore.Shared.Jobs[tb.name] then
                ...
                Core.GetJobs()
                while not QBCore.Shared.Jobs[tb.name] do 
                    Wait(10)
                end
                Core.GetJobs()
                return QBCore.Shared.Jobs[tb.name]
    ...
    DeleteJob = function(name)
        if Config.Debug then
            print("DeleteJob() skipped - don\'t worry about this print.")
        end
        --[[
        if ESX ~= nil then
    ...
    GetJobs = function()
        ...
        local jobs = ESX ~= nil and ESX.Jobs or QBCore ~= nil and QBCore.Shared.Jobs
        return jobs
    end,

    GetSharedAccount = function(society)
        if Config.SocietyAccount == 'esx_addonaccount' then
            return exports['esx_addonaccount']:GetSharedAccount(society)
        elseif Config.SocietyAccount == 'qb-banking' then
            return exports['qb-banking']:GetAccount(society)
        elseif Config.SocietyAccount == 'qb-management' then
            return exports['qb-management']:GetAccount(society)
        elseif Config.SocietyAccount == 'custom' then
            -- insert export to get account data, just like I did above.
        end
    end,

    CreateSharedAccount = function(society, startBalance)
        local account = Core.GetSharedAccount(society.name)
        if not account or account == nil then -- create shared account:
            if Config.SocietyAccount == 'esx_addonaccount' then
                return exports['esx_addonaccount']:AddSharedAccount(society, startBalance)
            elseif Config.SocietyAccount == 'qb-banking' then
                exports['qb-banking']:CreateJobAccount(society.name, startBalance)
            elseif Config.SocietyAccount == 'qb-management' then
                exports['qb-management']:CreateManagementAccount(society.name, startBalance)
            elseif Config.SocietyAccount == 'custom' then
                -- insert export to get account data, just like I did above.
            end
            account = Core.GetSharedAccount(society.name)
        end
        return account
    end,

    GetSharedAccountBalance = function(society)
        local account = Core.GetSharedAccount(society)
        if account ~= nil then 
            if Config.SocietyAccount == 'esx_addonaccount' then
            ...
            elseif Config.SocietyAccount == 'qb-management' then
                return account or 0
            elseif Config.SocietyAccount == 'custom' then
                -- insert export to return account balance
            end
        end
        return 0
    end,

    AddSharedAccountMoney = function(society, m)
        if Config.SocietyAccount == 'esx_addonaccount' then
        ...
        elseif Config.SocietyAccount == 'qb-banking' or Config.SocietyAccount == 'qb-management' then
            exports[tostring(Config.SocietyAccount)]:AddMoney(society, m)
        elseif Config.SocietyAccount == 'custom' then
            -- insert export to return account balance
        end
    end,

    RemoveSharedAccountMoney = function(society, m)
        if Config.SocietyAccount == 'esx_addonaccount' then
            TriggerEvent('esx_addonaccount:getSharedAccount', society, function(account)
                if account ~= nil then 
                    account.removeMoney(m)
                end
            end)
        elseif Config.SocietyAccount == 'qb-banking' or Config.SocietyAccount == 'qb-management' then
            exports[tostring(Config.SocietyAccount)]:RemoveMoney(society, m)
        elseif Config.SocietyAccount == 'custom' then
            -- insert export to return account balance
        end
    end,

    SetSharedAccountMoney = function(society, m)
        if Config.SocietyAccount == 'esx_addonaccount' then
            TriggerEvent('esx_addonaccount:getSharedAccount', society, function(account)
                if account ~= nil then
                    account.setMoney(m)
        ...
        elseif Config.SocietyAccount == 'qb-banking' or Config.SocietyAccount == 'qb-management' then
            local balance = Core.GetSharedAccountBalance(society)
            Core.RemoveSharedAccountMoney(society, balance)
            Wait(100)
            Core.AddSharedAccountMoney(society, m)
        elseif Config.SocietyAccount == 'custom' then
            -- insert export to return account balance
        end
    end,
            ...
            if results and next(results) then
                for i = 1, #results do
                    ...
                    if Config.Garage == 'codeM-garage' then
                        vehicles[i].stored = results[i].stored
                        vehicles[i].garage = results[i].parking 
                    end
    
                    if Config.Garage == 'qs-advancedgarages' then
                        vehicles[i].stored = results[i].stored
                        vehicles[i].garage = results[i].garage 
                    end
...
RegisterServerEvent('t1ger_lib:server:openInventory')
AddEventHandler('t1ger_lib:server:openInventory', function(id, label, slots, weight, owner)
