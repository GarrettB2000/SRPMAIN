...
function AddMoneyToAccount(account, amount, isNotRent)
    ...
    if source then
        ...
        if not isNotRent then
            TriggerClientEvent('qb-houses:sendTextMessage', source, Lang('HOUSING_NOTIFICATION_RENT_PAYMENT') .. amount, 'error')
        end
    else
        -- Fetch the player's account data from the database
        local result = MySQL.Sync.fetchAll('SELECT ' .. accountsColumn .. ' FROM ' .. userTable .. ' WHERE ' .. identifierColumn .. ' = ?', { account })
        
        -- Check if the result exists
        if not result or not result[1] then
            print('AddMoneyToAccount: Account not found: ' .. account)
            return
        end
        
        -- Fetch accounts based on the framework
        local accounts = result[1].money and json.decode(result[1].money) or json.decode(result[1].accounts)
        
        -- If accounts is nil or doesn't contain the bank value, handle the error
        if not accounts or not accounts.bank then
            print('AddMoneyToAccount: Invalid account data for account: ' .. account)
            return
        end
