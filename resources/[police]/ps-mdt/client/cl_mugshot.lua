...
-- Mugshot location  ( qabz  pris)
    x = 1843.3829
    y = 2594.3588
    z = 46.5144
    r = {x = 0.0, y = 0.0, z = 268.10269}
    suspectheading = 85.566711
    suspectx = 1844.7076
    suspecty = 2594.5046
    suspectz = 45.016242
...
RegisterNUICallback("sendToJail", function(data, cb)
    -- Extract citizenId and sentence from the data
    local citizenId = data.citizenId
    local sentence = data.sentence

    -- Check if sentence is valid
    if sentence <= 0 then
        print("Invalid sentence duration")
        cb({ success = false, message = "Invalid sentence duration" })
        return
    end

    -- Trigger the MugShot webhook
    QBCore.Functions.TriggerCallback('ps-mdt:server:MugShotWebhook', function(webhookUrl, apiKey)
        local webhookUrl = Config.MugShotWebhook and webhookUrl or 'https://api.fivemerr.com/v1/media/images'

        -- Check if webhookUrl is valid
        if webhookUrl == '' then
            print("No valid webhook URL")
            cb({ success = false, message = "No valid webhook URL" })
            return
        end

        -- Get the player id from the citizenId
        local p = promise.new()
        ...
        if targetSourceId == nil or targetSourceId == 0 then
            print("Invalid player ID")
            cb({ success = false, message = "Invalid player ID" })
            return
        end

        -- Trigger the mugshot event if enabled in config
        if Config.UseCQCMugshot then
            TriggerServerEvent('cqc-mugshot:server:triggerSuspect', targetSourceId)
            Citizen.Wait(5000) -- Wait for mugshot to be processed
        end
        ...
        -- Confirm the action
        cb({ success = true, message = "Player sent to jail" })
    end)
end)
