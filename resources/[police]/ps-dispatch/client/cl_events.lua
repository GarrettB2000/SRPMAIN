local WeaponTable = {
    ...
    [2132975508]  = "CLASS 3: Bullup Rifle",
    ...
    [-1654528753] = "CLASS 3: Bullup Shotgun",
    ...
    [GetHashKey("weapon_glock18c")] = "CLASS 2: Glock 18C",
    [125959754]   = "CLASS 5: Compactlauncher"
}
...
local function InjuriedPerson()
    ...
    TriggerServerEvent("dispatch:server:notify", {
        ...
        job = {"EMS", "ambulance", "police"} -- type or jobs that will get the alerts
    })
end
...
local function BobcatRobbery(camId)
    local currentPos = GetEntityCoords(PlayerPedId())
    local locationInfo = getStreetandZone(currentPos)
    local gender = GetPedGender()
    TriggerServerEvent("dispatch:server:notify", {
...
local function OilRig()
    ...
    TriggerServerEvent("dispatch:server:notify", {
        dispatchcodename = "oilrig", -- has to match the codes in sv_dispatchcodes.lua so that it generates the right blip
        dispatchCode = "10-68",
        firstStreet = '',
        gender = gender,
        model = nil,
        ...
        dispatchMessage = 'Oil Rig Robbery', 
        job = { 
            "police", 
            "bcso",
            "sasp",
            "dea"
        } -- jobs that will get the alerts
    })
...
local function OfficerAssistance()
    local plyData = QBCore.Functions.GetPlayerData()
    local currentPos = GetEntityCoords(PlayerPedId())
    local locationInfo = getStreetandZone(currentPos)
    local callsign = QBCore.Functions.GetPlayerData().metadata["callsign"]
    TriggerServerEvent("dispatch:server:notify",{
        dispatchcodename = "officerassistance", -- has to match the codes in sv_dispatchcodes.lua so that it generates the right blip
        dispatchCode = "10-32",
        firstStreet = locationInfo,
        name = "COP - " .. plyData.charinfo.firstname:sub(1, 1):upper() .. plyData.charinfo.firstname:sub(2) .. " " .. plyData.charinfo.lastname:sub(1, 1):upper() .. plyData.charinfo.lastname:sub(2),
        model = nil,
        ...
        dispatchMessage = _U('officerassistance'), -- message
        job = {"police"} -- type or jobs that will get the alerts
    })
end

exports('OfficerAssistance', OfficerAssistance)

RegisterNetEvent("ps-dispatch:client:OfficerAssistance", function ()
    OfficerAssistance()
end)
