    ...
    if coords then
        ...
        QBCore.Functions.TriggerCallback('police:server:PayForVehicle', function(result, plate)
            ...
            QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
                ...
                -- Apply performance upgrades and extras
                if Config.EnableMods then
                    PerformanceUpgradeVehicle(veh)
                end
                if Config.EnableExtras and Config.CarExtras.extras ~= nil then
                ...
                -- Give player keys to the vehicle
                TriggerEvent("vehiclekeys:client:SetOwner", plate)

                -- Ensure the vehicle is properly set up
                SetVehicleEngineOn(veh, true, true)

                -- Additional logic for job vehicles
                if Config.OwnedPoliceCars then
                    -- Add any necessary job-specific logic here
                end
...
local function HeliGarage(currentSelection)
    ...
    -- Ensure authorizedHelis is valid
    if not authorizedHelis then
        print("No helicopters found for current selection: " .. currentSelection)
        return
    end

    -- Check if Heli is initialized and has valid entries
    if Heli and next(Heli) then
        ...
        -- Handle the case where Heli is nil or empty
        for veh, data in pairs(authorizedHelis) do
            for _, v in pairs(data.ranks) do
                if v == PlayerJob.grade.level then
                    local vehprice, pricetext
