...
QBCore.Functions.CreateCallback("core_evidence:getReport",function(source, cb, reportId)
    exports.oxmysql:query("SELECT * FROM `evidence_storage` WHERE `id` = ? LIMIT 1", {reportId},
        function(report)
            cb(report)
        end)
    end
