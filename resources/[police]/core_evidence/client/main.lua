...
local function openLocker() 
    QBCore.Functions.TriggerCallback("core_evidence:getStorageData", function(data)
        SetNuiFocus(true, true)
...
RegisterNetEvent('core_evidence:client:openLocker', function()
    openLocker()
end)

exports('openLocker', openLocker)

local function showReport(reportId)
    if reportId == nil then return end

    QBCore.Functions.TriggerCallback("core_evidence:getReport", function(data)
        if data and #data > 0 then
            SetNuiFocus(true, true)
            SendNUIMessage(
                {
                    type = "showReport",
                    evidence = json.decode(data[1].data)
                }
            )
            if Config.PlayClipboardAnimation then
                TaskStartScenarioInPlace(ped, "WORLD_HUMAN_CLIPBOARD", 0, true)
...
RegisterNetEvent('core_evidence:client:showReport', function(reportId)
    showReport(reportId)
end)
