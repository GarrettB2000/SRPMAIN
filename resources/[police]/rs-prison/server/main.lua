...
RegisterNetEvent('prison:server:SaveJailItems', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    -- Check if jail items metadata is nil or empty, and save items if needed
    if Player.PlayerData.metadata["jailitems"] == nil or next(Player.PlayerData.metadata["jailitems"]) == nil then
        ...
        -- Player.Functions.AddMoney('cash', 80) -- Optionally, you can still add money
        -- Remove the line below to prevent clearing the inventory
